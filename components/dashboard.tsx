...
type View = "inicio" | "atenciones" | "usuarios" | "reportes" | "programas" | "pacientes"
...
export function Dashboard() {
  ...
  const navItems: NavItem[] = [
    ...
    { id: "atenciones", label: "Atenciones", icon: <ClipboardList className="h-5 w-5" /> },
  ]
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-sidebar text-sidebar-foreground transition-transform duration-300 lg:translate-x-0 h-[100dvh] overflow-y-auto ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        {/* Sidebar header */}
      ...
      <div className="flex flex-1 flex-col lg:ml-64 min-w-0">
        {/* Top bar */}
        <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center justify-between gap-4 border-b border-border bg-background px-4 lg:px-6">
          <button
